...
def verifica(x, w1, w2, w3, y, conteudo):
  ...
  try:
    with open('tabela_hould_out_4C.csv', 'r') as f:
      pass
  except IOError: tabela.append('Erros,Acertos,NINT1,NINT2,TAPR,TAPR1,Épocas,Acurácia,EMQ\n')

  tabela.append(str(cont)+','+str(acertos)+','+str(var.NINT1)+','+str(var.NINT2)+','+str(var.TAPR)+','+str(var.TAPR1)+','+str(var.MAXITER)+','+str(porcAcerto)+'%,'+str(var.emq)+'\n')

  arquivo = open('tabela_hould_out_4C.csv', 'a')
  ...
  conteudo.append('\nAcertos: 0.1 = '+str(act1)+', 0.2 = '+str(act2))
